...
#' @param all.radar valeus to print each gridlines
#' @param all.label valeus to print each gridlines label
...
#' @param gridline.min.linetype line type of minimum gridline or all gridline
#' @param gridline.mid.linetype line type of 'average' gridline
#' @param gridline.max.linetype line type of maximum gridline
#' @param gridline.min.colour colour of minimum gridline or all gridline
...
ggradar2 <- function(plot.data,
                    ...
                    grid.min = ifelse(length(all.radar),min(all.radar),0), # 10,
                    grid.mid = 0.5, # 50,
                    grid.max = ifelse(length(all.radar),max(all.radar),1), # 100,
  ...
  #all.radar <- sort(all.radar)
  if(!is.null(all.radar)){
     if(is.null(all.label)){
        all.label <- all.radar
     }
     gridline <- as.list(c(1:length(all.radar)))
     for(i in gridline){gridline[[i]] <- list()}
     for(i in 1:length(all.radar)){
        gridline[[i]][['path']] <- ggradar:::funcCircleCoords(c(0, 0), all.radar[i] + abs(centre.y), npoints = 360)
        gridline[[i]][['label']] <- data.frame(x = gridline.label.offset,
                                               y = all.radar[i] + abs(centre.y),
                                               text = as.character(all.label[i]))
     }
  }else{
     gridline <- NULL
  ...
  if(!is.null(all.radar)){
     for(i in 1:length(all.radar)){
        if(i == length(all.radar)){
           base <- base + geom_path(data = gridline[[i]][['path']], aes(x = x, y = y),
                                    size = grid.line.width)
        }else{
           base <- base + geom_path(data = gridline[[i]][['path']], aes(x = x, y = y), lty = gridline.min.linetype,
                                    colour = gridline.min.colour, size = grid.line.width)
        }
     }
  }else{
     base <- base + geom_path(
  ...
  if(!is.null(all.radar)){
     idx <- length(all.radar)
     base <- base + geom_polygon(data = gridline[[idx]][['path']], aes(x,y),fill = background.circle.colour,alpha = background.circle.transparency)
  }else{
  ...
  if(!is.null(all.radar)){
     addlab <- function(idx){
       p <- geom_text(aes(x = x, y = y, label = as.character(all.label[idx])), data = gridline[[idx]][['label']],
                          size = grid.label.size * 0.45, hjust = 0.03, family = font.radar)
       return(p)
     }
     base <- base + lapply(1:length(all.radar), addlab)
  }else{
